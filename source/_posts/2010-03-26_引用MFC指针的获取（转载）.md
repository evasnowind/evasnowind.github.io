...
categories: ["C/C++"]
tags: ["C/C++"]
source: "http://prayerlaputa.com/?p=420"
description: "围绕引用MFC指针的获取整理相关实践、经验与要点。"
---

\
 \
转自：http://hi.baidu.com.sixxs.org/eagle\_shuai/blog/item/325c3abfb763800619d81f90.html\
chilli 的 MFC指针的获取
...
这个很简单，一句话搞定：\
CMyApp\* pApp=(CMyApp\*)AfxGetApp();\
在类CWinThread里面有一个公有的成员变量：CWnd\* m\_pMainWnd; 它存在的主要目的就是提供我们获得CWnd指针，我们可以利用它来达到我们的目的：\
...
if(!pView->IsKindOf(pClass)){\
AfxMessageBox(“Connt Locate the View.rn “);\
return NULL;\
}\
return pView;\
}\
...
RUNTIME\_CLASS是一个宏，可以简单地理解它的作用：将类的名字转化为\
...
对于单文档中也可以加入多个文档模板，但是一般的开发就使用MDI方式开发\

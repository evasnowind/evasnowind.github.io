---
title: "Java 根据 URL 获取输入流以及下载文件"
date: "2019-12-05"
categories: ["Java", "网络"]
tags: ["Java", "网络编程"]
source: "http://prayerlaputa.com/?p=723"
description: "整理 Java 中通过 URL 获取输入流并将远程内容下载到本地的常见写法。"
---

这篇文章整理 Java 中根据 URL 获取输入流、再将远程内容下载到本地的常见写法。

原始示例可参考：[JAVA根据URL网址获取输入流](https://blog.csdn.net/u012012240/article/details/85078929)
...
其中 `Content-Type` 可以用 MediaType 中的常量自行控制；`Content-Disposition` 则看需求，如果需要直接打开文件，则传 `"inline"`，若是想下载则传 `"attachment"`。

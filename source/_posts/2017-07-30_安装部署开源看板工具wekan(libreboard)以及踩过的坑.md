...
description: "一直很想使用看板工具来管理工作任务，市面上trello/teambition等有很多有用的工具，方便更新工作状态。然而，由于公司内网没法连外网，只能自己部署一套了。"
---
...
- ubuntu虚拟机中用snap安装了wekan，配置 `root_url="localhost:xxx"` 后，我访问宿主机 IP 没问题，但点击 card 后，域名会由宿主机 IP 变成 localhost。
  - root\_url 没配置好。我是将虚拟机部署在宿主机上，并用 VirtualBox 做了端口映射，但一开始还是把 root\_url 配成了 localhost:xxx。实践证明，Wekan 会把这个参数作为点击 card 时的根域名，因此我重新将 root\_url 配置为宿主机 IP:xxx，之后就正常了。
---
title: "MySQL 中判断字段是否包含数字，或者是否为纯数字"
date: "2020-02-26"
categories: ["数据库"]
tags: ["MySQL"]
source: "http://prayerlaputa.com/?p=772"
description: "整理 MySQL 中几个常见判断场景，包括是否包含数字、是否为纯数字，以及基于 like / FIND_IN_SET / REGEXP 的写法。"
---

这篇文章整理几个在数据清洗或条件筛选里比较常见的判断场景，顺手汇总成一页备查。

- 各种场景
  - 判断字段是否包含数字
  - 使用 like 模糊查询包含某个数字
  - 使用 mysql 原生函数 FIND_IN_SET 查询包含某个数字
  - 使用 regexp 正则匹配纯数字
  - 使用 regexp 正则匹配字段值不包含数字
- 参考资料

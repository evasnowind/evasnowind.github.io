---
title: "源码分析之 Eureka 客户端源码解析"
date: "2020-09-22"
categories: ["Java", "Spring Cloud"]
tags: ["Spring Cloud", "源码分析"]
source: "http://prayerlaputa.com/?p=881"
description: "围绕 Eureka Client 的自动配置、服务注册、拉取注册表与心跳机制，梳理其核心源码实现。"
---

## 前言

本文分析的代码版本为 Spring Boot / Spring Cloud 2.2.6。

## 预备知识

### 一些注解的说明

Eureka 中用到了几个比较关键的注解，它们对整体自动配置流程有很大帮助。

##### @ConfigurationProperties("eureka.instance")

<!-- more -->

表示从外部配置文件中（properties 或 yml 文件）读取 `eureka.instance` 对应的配置。
...
![](/images/eureka-architecture-1024x548_c59397f0.png)

## eureka客户端源码

### eureka客户端工作流程

![](/images/eureka-client-work-flow-1024x598_c91e0794.png)

下面将简要说明上图的过程。

eureka客户端比较关键的类如下：

![](/images/eureka-client-main-class-300x150_5e20efa1.png)
...
进入getAndStoreFullRegistry() 方法，有一方法：eurekaTransport.queryClient.getApplications。
  ...
  webResource 地址为：`http://root:root@eureka-7900:7900/eureka/apps/`，此端点用于获取 server 中所有的注册表信息。
...
![](/images/eureka-client-startup-work-flow-1024x728_79011726.png)
...
![](/images/spring-cloud-netflix-eureka-client-metai_8e79bf68.png)

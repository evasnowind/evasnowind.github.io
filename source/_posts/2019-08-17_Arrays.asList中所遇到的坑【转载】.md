...
description: "最近在项目上线的时候发现一个问题，从后台报错日志看：java.lang.UnsupportedOperationException异常 从代码定位来看，原来是使用了Arrays.asList(。"
---

原文链接：<https://www.cnblogs.com/wang-meng/p/f1532cf23ce049ce63b4bdd62d53659d.html>

<!-- more -->

## 前言

最近在项目上线时遇到一个问题，从后台日志看抛出了 **java.lang.UnsupportedOperationException**。
定位代码后发现，问题出在使用 `Arrays.asList()` 将数组转换为 List 之后，又对返回的 List 执行了 `add()` 和 `remove()` 操作。

借这个问题，顺便整理一下 `Arrays` 相关的几个常见坑。
